                                    ...
                                    .then(data => {
                                        ...
                                        if (data && data.issues && data.issues.length > 0) {
                                            ...
                                            data.issues.forEach(issue => {
                                                ...
                                                newRow.innerHTML = `<td>${issue.key}</td><td><a href="https://asset360.morganserver.com/asset/view/?id=${issue.fields.labels[1]}" target="_blank" class="" style="">${issue.fields.labels[0]}</a></td><td>${issue.fields.summary}</td><td>${issue.fields.issuetype.name}</td><td>${new Date(issue.fields.created).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</td><td>${new Date(issue.fields.updated).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</td><td><a href="https://garrett-morgan.atlassian.net/browse/${issue.key}" target="_blank" class="badge text-bg-primary text-decoration-none" style="font-size: 14px;">Visit</a></td>`;
